...
const commentsListTemplate = document.getElementById('comments-list');

function commentsList(){
  commentsListTemplate.innerHTML=`${myObj.comments.map(comments).join("")}`;
  editComment()
  updateComment()
  deleteComment()
  scoreUp()
    ...
    <div class="container">
      ...
      <div class="comment-wrapper">
        <div class="comment-header">
          ...
          <div class="reply-btn-wrapper">${showBtns(comment)}</div>
        </div>
        <div class="comment-text">
          <p id="text" contenteditable="false">${comment.content}</p>
        </div>      
        <div class="update-btn"></div>        
      </div>
    </div>
    <div class="new-box"></div>
    <div class="replyList">${comment.replies.map(replies).join("")}</div>
    ...
    <div class="container">
      ...
      <div class="comment-wrapper">
        <div class="comment-header">
          ...
          <div class="reply-btn-wrapper">${showBtns(reply)}</div>
        </div>
        <div class="comment-text">
...
function showBtns(comment){ 
  if(comment.user.username == myObj.currentUser.username){
    return`
...
const sendComment = document.getElementById('send-comment-btn').addEventListener('click', function(){
  addComment()
});
...
function addComment(){
  ...
  let newComment = {
    "id": 4,
      ...
      "user": {
        "image": { 
          "png": `${myObj.currentUser.image.png}`,
          "webp": `${myObj.currentUser.image.webp}`
        },
        "username": `${myObj.currentUser.username}`
      },
      "replies": []
  }

  myObj.comments.push(newComment)
  commentsList()
};
